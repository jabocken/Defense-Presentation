...
\usepackage{relsize}               % Necessary for \gls{cpp} formatting
...
\usepackage{tikzducks}             % For fun

\usepackage{booktabs}              % For nicer-looking tables
\usepackage{makecell}              % For table heading cells and cells with line breaks
\usepackage{threeparttable}        % For tables with notes

% Necessary due to some breakage with \thead/S table column interaction 
...
% Using listings directly inside certain environments doesn't always work so we sometimes need to save it in a box first
\newsavebox\lstbox
...
\begin{document}
  ...
  \appendix % Not sure if this is really right but oh well, it excludes the sections from the outline which is good, but it also causes a hyperref warning about a "\translate" in a PDF string due to Appendix being a localized name, but there's no visible issues so oh well on that too?
